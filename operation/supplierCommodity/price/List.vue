    ...
    <div class="price-management-list" v-loading="loading">
        ...
        <div class="list-table">
            <el-table ref="multipleTable" :data="table.data">
                ...
                <el-table-column prop="province" label="供应省份" width="80"></el-table-column>
                ...
                <el-table-column label="审核状态" width="160px">
                    <template slot-scope="scope">
                        {{stateMap[scope.row.checkState]}}
                    </template>
                </el-table-column>
                <el-table-column label="比上次价格" width="100">
                    <template slot-scope="scope">
        ...
        <el-dialog :title="addDialog.model.id ? '编辑价格方案':'添加价格方案'" :visible.sync="addDialog.visible" width="500px"
            ...
            <el-form :model="addDialog.model" ref="addForm" :rules="validate" label-width="120px">
                <el-form-item label="商品：" prop="goodsId">
                    <el-select v-model="addDialog.model.goodsId" placeholder="请选择" :disabled="!!addDialog.model.id">
        ...
        methods: {
            ...
            provinceBlur() {
                if (this.addDialog.model.provinceIds.includes(-1)) {
                    this.addDialog.model.provinceIds = [-1];
                }
            },
            remove(row) {
                this.$confirm('确认删除该价格?', '确认', {
                    ...
                    this.operateAjax("/admin/goodsonsale/delete", data);
                }).catch(() => {
                });
            },
            ...
            fetchList() {
                ...
                this.$axios({
                    method: "post",
                    url: this.$basePath + "/admin/goodsonsale/list",
                    params: {
                        length: 10,
            ...
            fetchDownTime() {
                this.$axios({
                    method: "get",
                    url: this.$basePath + "/admin/goodsonsale/downTime"
                }).then((response) => {
                    response = response.data;
                    if (response.code == 0) {
            ...
            fetchGoods() {
                this.$axios({
                    method: "get",
                    url: this.$basePath + "/admin/goods/desc/list"
                }).then((response) => {
                    response = response.data;
                    if (response.code == 0) {
            ...
            addSubmit() {
                ...
                this.$axios({
                    method: "post",
                    url: this.$basePath + (this.addDialog.model.id ? "/admin/goodsonsale/edit" : "/admin/goodsonsale/save"),
                    data: data
                }).then((response) => {
                    this.loading = false;
                    response = response.data;
            ...
            adjustSubmit() {
                ...
                this.$axios({
                    method: "post",
                    url: this.$basePath + "/admin/goodsonsale/save",
                    data: data
                }).then((response) => {
                    this.loading = false;
                    response = response.data;
